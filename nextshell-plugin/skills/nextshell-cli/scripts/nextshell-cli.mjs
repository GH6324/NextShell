...
var coerceFlagValue = (raw) => {
  ...
  const looksJson = trimmed === "true" || trimmed === "false" || trimmed === "null" || /^-?\d+(\.\d+)?$/.test(trimmed) || trimmed.startsWith("{") || trimmed.startsWith("[") || trimmed.startsWith('"');
  if (!looksJson) {
    return raw;
  }
  try {
...
`);
  ...
  try {
    const timeoutMs = invocation.timeoutSec !== null ? invocation.timeoutSec * 1e3 : DEFAULT_CALL_TIMEOUT_MS;
